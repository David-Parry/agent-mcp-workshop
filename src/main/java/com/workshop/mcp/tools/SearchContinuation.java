...
public record SearchContinuation(String keyword, String stage) {
    ...
    /** The server has asked the client for its roots and is awaiting the answer. */
    public static final String STAGE_ROOTS = "roots";

    /** Roots came back empty, so the server has asked the user for a directory. */
    public static final String STAGE_DIRECTORY = "directory";

    /** Key under which the embedded {@code roots/list} answer comes back. */
    public static final String KEY_ROOTS = "search_roots";
    ...
    /**
     * Creates the state for a pending roots question.
     *
     * @param keyword the keyword being searched for
     * @return the continuation state
     */
    public static SearchContinuation awaitingRoots(String keyword) {
        return new SearchContinuation(keyword, STAGE_ROOTS);
    }
